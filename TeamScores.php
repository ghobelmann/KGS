...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      echo '<font size="6">   ';  
      echo '<a href="TeamScores.php?classification='.$row['classification'].'&state='.$state.'&db='.$db.'">'.$row['classification'].'</font></a>';
      echo '</font>';
    }        
} else {                      
...
$sql = "SELECT DISTINCT `spam` FROM `scores3` WHERE spam = '1'";
...
$sql = "SELECT scores3.tmid, scores3.spam, scores3.tid, 
team.school, team.classification FROM scores3
INNER JOIN team on scores3.tmid = team.tmid
WHERE scores3.spam = '1'
AND team.state = '$state'
AND scores3.status = '1' " ;
...
$sql2 = "SELECT pid, tmid, avg (scores3.total) as total, spam  FROM `scores3` 
