...
echo "<H2 style='color:red;'>The time is " . date("h:i:sa");
?>
      <font size="6"
          color="rgb(128, 0, 0)">
            High School
        </font><br>
          <button class="button button2"><a href="teacherReport.php">Teacher Reports</a></button>
          <button class="button button1"><a href="subReport.php">Sub Reports</a></button>
    ...
    <div class="col s12 m6">
      <div class="card large blue-grey darken-1">
        <div class="card-content white-text">
          ...
          <div class="section">
          <form action="connectsubhs.php" class="col s12 m12">
...
<!-- Date Entry Field-->
  ...
  <div class="input-field col s6 m12">
          ...
          $date = date('Y/m/d');
          //echo $date;
           if($r_set = $conn->query("SELECT tsick_id, name from total_sick WHERE building = '2' ORDER by name ASC")){
...
<?php
           if($r_set = $conn->query("SELECT sub_name, sub_id from personnel ORDER by sub_name ASC")){
...
echo "<select name='pay' class='form-control' id='pay'>";
while ($row = $r_set->fetch_assoc()) {
...
echo $connection->error;
 ...
 ?>
  ...
  <!-- Leave Type Field-->
    <?php
           if($r_set = $conn->query("SELECT * from department ORDER by dept_id ASC")){

echo "<select name='dept_id' class='form-control' id='dept_id'>";
while ($row = $r_set->fetch_assoc()) {
...
$sql1 = "SELECT *, (sickdays.id) as id, (personnel.sub_id) as pid FROM sickdays 
