...
$sql1 = "SELECT *, (sickdays.id) as id, (personnel.per_id) as pid, 
        ...
        AND sickdays.pay_id IN ('1', '2')
        ORDER BY school asc, sickdays.date asc
        ";
...
while($row = mysqli_fetch_array($result))
{
  ...
  echo '<td><a href="subQuery.php?sub_id='.$row['sub_id'].'">'.$row['sub_name'].'</font></a></td>';
        ...
        echo "<td  style='font-size: 12px; text-align:CENTER'>$row[type]-$row[school]</td>";
