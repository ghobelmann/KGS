...
$sql1 = "SELECT *, (sickdays.id) as id, (personnel.per_id) as pid, tc_pay,
...
while($row = mysqli_fetch_array($result))
{
  ...
  //Daily Sub
  echo '<tr><td><a href="editTC.php?id='.$row['id'].'">'.$row['id'].'</font></a></td>';
  echo '<td><a href="teacherQuery.php?name_id='.$row['name_id'].'">'.$row['name'].'</font></a></td>';
  echo '<td><a href="tcQuery.php?tc_id='.$row['tc_id'].'">'.$row['tc_name'].'</font></a></td>';
        echo "<td  style='font-size: 12px; text-align:CENTER'>$row[date]</td>";
        echo "<td  style='font-size: 12px; text-align:CENTER'>$row[tc_pay]</td>";
        echo "<th style='font-size: 12px; text-align:left'>Notes:  $row[notes]</td></tr>";
