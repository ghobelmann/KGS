...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
 ...
 <table width="400" border="1" cellpadding="1">
  <tr>
    <td><h3 align="center" class="style2"> Points per Week </h3>
      <?php

// Query for Driving Stats and Yardage
$query = "SELECT sum(points) as points, count(mtch) as mtch, date from scores group by date order by date desc"; 
$result = mysql_query($query) or die(mysql_error());
	
	
echo "<table border='1'>";
//Table setup for Driving info
echo "<tr> <th>Points</th> <th>Date</th><th>Number of Players</th><th>avg</th>";
...
while($row = mysql_fetch_array( $result )) {
	...
	echo $row['points'];	
	echo "</td><td><CENTER>"; 
	echo '<a href="weeklyresults.php?date='.$row['date'].'"><font color=black>'.$row['date'].'</font></a>';
	echo "</td><td><CENTER>"; 
	echo $row['mtch'];
	echo "</td><td><CENTER>"; 
	echo $row['points']/$row['mtch'];	
	echo "</td></tr><CENTER>";  
} 
echo "</table>";
?></td>
