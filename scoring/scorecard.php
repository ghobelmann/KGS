  ...
  <!-- <input class='my-btn' type='button' onclick='location.reload();' value='Calculate Page' />  </h3> -->
    ...
    <table id="editable_table" class="table table-bordered table-striped">
     <thead>
      ...
      <th>F</th><th>B</th> <th>T</th> <th>Status</th><th>Edit</th>
      </tr>
     </thead>
     <tbody>    <hr> 
     <?php    
...
// echo $tid;
    ...
    $sql = "SELECT count(scores.pid) as count, scores.round, scores.id, scores.pid, scores.tmid, scores.tid, sum(scores.front) as front, 
...
// echo $tid;
    ...
    $sql = "SELECT count(scores.pid) as count, scores.id, scores.pid, scores.tmid, scores.tid, 
    sum(scores.total) as total, scores.last6, scores.last3,
   roster.pid, roster.player_1, roster.grade, team.tmid, team.abv
   FROM `scores`
   INNER JOIN roster on scores.pid = roster.pid 
   LEFT JOIN team on scores.tmid = team.tmid
...
echo '<td colspan="3" align="center"><h3>Total</h3>';
echo "<tr><th><center> Place </th><th><center>Name </th> 
<th><center> Abv</th><th><center>Total</th></b> </tr>";
// keeps getting the next row until there are no more to get
 $i=1;
while($row = mysqli_fetch_array( $result )) {
	// Print out the contents of each row into a table
...
$sql = "SELECT scores.total, scores.tmid, team.school, team.abv FROM scores
...
echo '<tr><th>Place</th><th><center>Team</th><th>Score</th></tr>';

asort($totals); $place=1; 
foreach ($totals as $key => $val)  { 
    echo "<tr><td>".$place."</td><td><center>".$key."</td><td><center>".$val."</td></tr>";
     $place++;
 }  
