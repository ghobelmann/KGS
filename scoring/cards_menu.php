...
echo '<tr><th colspan="30" bgcolor=#000000><center><h2><font font color ="white">Round 1 Score Cards.</h2></center>';
echo '<h4><a href="../index.php" font color ="white">Home</a></h4></th></tr>';
// keeps getting the next row until there are no more to get
while($row = mysqli_fetch_array( $result )) {
    ...
    echo '<td bgcolor=green><center> <h4>';
   	echo '<a href="scorecard.php?tid='.$row['tid'].'
       &card='.$row['card'].'&round=1">
...
while($row = mysqli_fetch_array( $result )) {
	...
	echo '<a href="scorecard2.php?tid='.$row['tid'].'&card='.$row['card'].'&round=2">'.$row['card'].'</font></a>';
	echo "</td>"; 
...
echo "<table width='100%' border='2'>";
echo '<tr><th colspan="30" bgcolor=#000000><center><h2><font font color ="white">Round 3 Score Cards.</h2></center></th></tr>';
// keeps getting the next row until there are no more to get
while($row = mysqli_fetch_array( $result )) {
	// Print out the contents of each row into a table
	
	echo "<td bgcolor=#f0e2bd><center> <h4>";
	echo '<a href="scorecard3.php?tid='.$row['tid'].'&card='.$row['card'].'&round=3">'.$row['card'].'</font></a>';
	echo "</td>"; 
...
echo "<table width='100%' border='2'>";
echo '<tr><th colspan="30" bgcolor=#000000><center><h2><font font color ="white">Round 4 Score Cards.</h2></center></th></tr>';
// keeps getting the next row until there are no more to get
while($row = mysqli_fetch_array( $result )) {
	// Print out the contents of each row into a table
	
	echo "<td bgcolor=#f0e2bd><center> <h4>";
	echo '<a href="scorecard4.php?tid='.$row['tid'].'&card='.$row['card'].'&round=4">'.$row['card'].'</font></a>';
	echo "</td>"; 
