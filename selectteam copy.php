...
if ($rounds > 1)  
       {  
       echo '<div class="col-lg-3 portfolio-item">';
          echo '<div class="card h-100">';
           echo ' <div class="card-body">';
            echo '<p class="card-text">'; 
    echo   "<h2>Round 2.</h2>";                     
$sql = "SELECT * FROM `tournament` WHERE id = '$id' AND rounds > 1 LIMIT 0 , 1 ";
if($results = @mysqli_query($conn,$sql))        
{                                               
$data = mysqli_fetch_assoc($results);           
//print_r ($data);                              
...
INNER JOIN `team` ON trnyteams.tmid = team.tmid
INNER JOIN `tournament` ON trnyteams.tid = tournament.id
  WHERE tournament.id = '$id' AND rounds > 1";
$result=mysqli_query($conn,$sql);
...
echo "Tournament is not setup for 2 Rounds.";
}
?>
...
while ($row=mysqli_fetch_array($result)) {
  ...
  echo "<tr><p>";
	echo '<a href="EnterbyTeam3.php?tmid='.$row['tmid'].'&tid='.$id.'">'.$row['school'].'</a></p>';
	echo "</tr>";
} }
else {
echo "Tournament is not setup for 3 Rounds.";
}
?>
       ...
       echo '<div class="col-lg-3 portfolio-item">';
          echo '<div class="card h-100">';
           echo ' <div class="card-body">';
            echo '<p class="card-text">'; 
    echo   "<h2>Round 4.</h2>";                     
$sql = "SELECT * FROM `tournament` WHERE id = '$id' AND rounds > 3 LIMIT 0 , 1 ";
if($results = @mysqli_query($conn,$sql))        
{                                               
$data = mysqli_fetch_assoc($results);           
//print_r ($data);                              
...
INNER JOIN `team` ON trnyteams.tmid = team.tmid
INNER JOIN `tournament` ON trnyteams.tid = tournament.id
  WHERE tournament.id = '$id' AND rounds > 3";
$result=mysqli_query($conn,$sql);
...
while ($row=mysqli_fetch_array($result)) {
  ...
  echo "<tr><p>";
	echo '<a href="EnterbyTeam4.php?tmid='.$row['tmid'].'&tid='.$id.'">'.$row['school'].'</a></p>';
	echo "</tr>";
} }
else {
echo "Tournament is not setup for 4 Rounds.";
}
?>
