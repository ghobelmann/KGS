       ...
       <a class="navbar-brand" href="index.php"> Home Page</a>
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item">
               <a class="nav-link" href="hbyhTourney.php?tid=<?php echo $tid; ?>&db=boys">Hole by Hole Results</a>
...
function get_round($tid,$round=1,$tied_teams=array(),$limit=4) {
    ...
    while($tid = mysqli_fetch_assoc($tournaments))
    {
        ...
        $sql = "SELECT scores2.total, scores2.tmid, scores2.status, 
        team.school FROM scores2
        INNER JOIN team on scores2.tmid = team.tmid
        WHERE scores2.tid = '".$tid['tid']."' 
        AND `round` = '".$round."' 
         AND `status` = '1'" ;
        ...
        {
            {
                ...
                $sql2 = "SELECT pid, tmid, total, tid, status  FROM `scores2`
...
?>
        ...
        <h4 class="card-header">C Team Results</h4>
...
<?php
    ...
    echo "<tr><td> Hole #</td><td>Average Score</td><td>Par</td>
                        <td>Yardage</td></tr>";
    for ($x = 1; $x <= 18; $x++) {
        $sql = "SELECT avg(hole$x) as h$x FROM scores2
        WHERE tid = $tid AND status <= '5'";
      $result = $conn->query($sql);
      if ($result->num_rows > 0) {
          // output data of each row
          while($row = $result->fetch_assoc()) {
            echo "<tr><td>Hole $x</td><td>";
...
?>
       ...
       </p>              <?php   echo "<img src='../../vendor/QR/php/qr_img.php?d=$output'>";   ?></p>      </div>
...
//    ($par + (((scores2.total-$rating)*113/$slope)))  as adj_score,
...
while($row = mysqli_fetch_array( $result )) {
   ...
   /*    $sql = "SELECT scores2.pid, scores2.tid, scores2.tmid, 
      sum(scores2.front) as front, 
      sum(scores2.back) as back, sum(scores2.total) as total, 
      scores2.last6, scores2.last3, rank 
      FROM (SELECT scores2.pid, scores2.tid, scores2.tmid, 
      sum(scores2.front) as front, sum(scores2.back) as back, 
      sum(scores2.total) as total, scores2.last6, scores2.last3, 
      @curRank := IF(@prevRank = total, 
      @curRank, @incRank) AS rank, 
      @incRank := @incRank +1, @prevRank := total 
      FROM scores2 sc, 
      (SELECT @curRank := 0, @prevRank := NULL, @incRank :=1) r 
      INNER JOIN roster on scores2.pid = roster.pid 
      LEFT JOIN team on scores2.tmid = team.tmid
      WHERE scores2.tid = '$tid' ORDER by total, back, last6, last3 ASC ) s 
      WHERE scores2.tid = '$tid' 
      GROUP by scores2.pid 
      ORDER BY `total`, `back`, `last6`, `last3` ASC";
            */
   $sql = "SELECT scores2.pid, scores2.tmid, scores2.tid, sum(scores2.total) as total,  scores2.last6, scores2.last3,
   ...
   LEFT JOIN team on scores2.tmid = team.tmid
   WHERE scores2.tid = '$tid'
    AND `status` <= '4'   GROUP by scores2.pid
  ORDER BY `round` DESC, `total`, `manualscore`, `back`, `last6`, `last3`, `abv` ASC"; 
...
$prev_score = -1;
   ...
   echo  "<table class='table table-hover' style='width:90%'><th style='width:10%'>" . Place . "</th><th style='width:35%'>" . Name . "</th>
        <th style='width:15%'>" . School . "</th><th style='width:25%'>" .  Score . "</th>";

foreach ($result as $row) {
    if ($row['total'] != $prev_score) {  //this score is not a tie
        $count = 0;
        $prev_score = $row['total'];
        echo  "<table style='width:90%'><td style='width:10%'>" . $rank . "</td>
        <td style='width:35%'>" . '<a href="playerStats.php?pid='.$row['pid'].'&db='.$db.'">'.$row['player_1'].'-'.$row['grade'].'</font></a>' . "</td>
        ...
        echo "<tr><td>T" . $tie_rank . "</td>
            <td style='width:25%'>" . '<a href="playerStats.php?pid='.$row['pid'].'&db='.$db.'">'.$row['player_1'].'-'.$row['grade'].'</font></a>' . "</td>
            <td style='width:25%'>" . '<a href="teamStats.php?tmid='.$row['tmid'].'&db='.$db.'">'.$row['school'].'</font></a>' . "</td>
            <td>" .  $row['total'] . "</td></tr>";
    }  
    $rank++;
