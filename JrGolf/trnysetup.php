...
?>    
   ...
   <?php
$sql="SELECT school, tmid, state FROM team WHERE `state` = '$state' ";
$result=mysqli_query($conn,$sql);
...
<h1>Setup New Tournament</h1>
  ...
  <h4>	<font color="white">
   State:   </font>
   	<input type="varchar" name="state" size="4"  value="ks" required/></h4>
...
<h4>	<font color="black">
    ...
    2. Name of Course  
      ...
      <input type="date" name="date" id="datepicker" size="10" value="<?php echo date('Y-m-d'); ?>" required/>   </h4>
		...
		<input type="hidden" id="trnyPar" name="par" size="4" value = "36"/>  
    
     <h4> 	
		<input type="hidden" name="yardage" size="4" value = "2500"/>
        <h4><font color="black">	
        </font>
		<input type="hidden" name="slope" size="4" value="113"/>  
    <br>

        <font color="black">
    7. Rating of Tees Played(Adjust this number to your course):
    	</font>
		<input type="hidden" name="rating" size="4" value="36"/> 

  
		<input type="hidden" name="leaderboard" size="4" value="99" required/> 
           <br> <br>
