...
?>
  ...
  <div class="row">
    <div class="col-sm-6 col-sm-left-1">
    <h4>If coaches entered tee times you can see them here</h4>
    <h3>Sorted By Team</h3>
<?php


$sql = "SELECT scores.id, scores.pid, scores.tmid, scores.card, scores.total,
scores.total,scores.status, scores.teetime, division.id,
roster.pid, roster.player_1 from scores 
INNER JOIN roster ON scores.pid = roster.pid 
INNER JOIN team ON scores.tmid = team.tmid 
WHERE tid = '$tournament' order by division.id, total"; 
...
echo "<table  width='75%'>";
echo "<tr> <th>Player</th> <th>Division</th><th>Card</th><th>Hole</th><th>Total</th></tr>";
...
while($row = mysqli_fetch_array( $result )) {
	...
	echo "</td><td>"; 
	echo '<a href="teamStats.php?tmid='.$row['tmid'].'">'.$row['divison'].'</font></a>';
