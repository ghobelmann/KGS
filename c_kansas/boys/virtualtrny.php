...
$query = "SELECT DISTINCT `tournament` FROM `season`  
WHERE tournament = '1'";
$tournaments=mysqli_query($conn,$query);
while($tournament = mysqli_fetch_assoc($tournaments))
{
$query = "SELECT s.*, avg(s.total) as total, r.pid, t.tmid, t.school
FROM `season` s
INNER JOIN roster r on s.pid = r.pid
LEFT JOIN team t on s.tmid = t.tmid
WHERE  total > 0  AND
s.tmid IN ('$team1') OR s.tmid IN('$team2') OR s.tmid IN('$team3') OR s.tmid IN('$team4') OR s.tmid IN('$team5')
...
$query2 = "SELECT pid, tmid, avg(total) as total  FROM `season` 
WHERE  total > 0  AND
 `tmid` = '".$team['tmid']."' 
AND `tid` = '1'
