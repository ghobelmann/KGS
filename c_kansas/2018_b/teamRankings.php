  ...
  <body>
    ...
    <div class="container">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href='teamRankings.php?classification=3A'> 3A </a> </li>
        <li class="breadcrumb-item"><a href='teamRankings.php?classification=2A'> 2A </a>  </li>
        <li class="breadcrumb-item"><a href='teamRankings.php?classification=1A'> 1A </a>  </li>
        <li class="breadcrumb-item"><a href='teamRankings.php?classification=sand'> Sand </a> </li>
...
$sql = "SELECT scores.tmid, scores.spam, team.paid, 
...
AND dq != 'DQ' AND dq != 'WD' 
AND `non_varsity` = 'no' and total > '0'" ;
...
WHERE `tmid` LIKE '".$team['tmid']."' AND `spam` LIKE '1' and total > '0'
AND `noteam` <> 'yes' AND `jv` != 'JV'  
AND dq != 'DQ' AND dq != 'WD' 
 AND `non_varsity` = 'no'
...
avg(((s.total-rating)*113/slope)) as adj,
avg((((s.total-rating)*113/slope))+ par) as adj_score
FROM scores s 
INNER JOIN roster r ON s.pid = r.pid
LEFT JOIN team t on s.tmid = t.tmid
WHERE t.classification LIKE '$classification' AND s.total >'0' AND `non_varsity` = 'no'
