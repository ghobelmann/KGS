...
AND `noteam` <> 'yes' AND `jv` != 'JV' AND cteam != 'JV2' AND `dq` != 'DQ' AND `dq` != 'WD'
ORDER BY `total` ASC";
...
$sql2 = "SELECT pid, tmid, total, tid  FROM `scores` 
WHERE `tmid` LIKE '".$team['tmid']."' AND `tid` LIKE '".$tid['tid']."' 
AND `noteam` <> 'yes' AND `cteam` = 'JV2' AND `dq` != 'DQ' AND `dq` != 'WD'
ORDER BY `total` ASC";
$result2=mysqli_query($conn,$sql2);}
...
echo '<table>';
echo '<tr><th valign="top">Team</td><th valign="top">Score</td><tr>';
asort($totals);
foreach ($totals as $key => $val) {
    echo "<tr><td>".$key."</td><td>".$val."</td></tr>";
 }
...
$query = "SELECT scores.pid, scores.tmid, scores.tid, scores.front,
  scores.jv, scores.cteam, 
   ...
   WHERE scores.tid = '$tid' AND `dq` != 'DQ'   AND `dq` != 'WD'
