...
while($row = mysqli_fetch_array( $result )) {
	...
	echo "<td><center>";
	echo '<a href="cardtotal.php?tid='.$trny.'&card='.$row['card'].'">'.$row['card'].'</font></a>';
	echo "</td>"; 
...
echo "</table>";
 ...
 <?php
//Submit Query to MySql Database
$sql = "SELECT scores.tid, scores.uid, tournament.id 
from scores
INNER JOIN tournament ON scores.tid = tournament.id
WHERE '$trny' = tournament.id GROUP by scores.tid ORDER by scores.tmid ASC, scores.pid ASC";
$result = mysqli_query($conn,$sql)
or die ('Error in Query Try Again:--' . mysqli_error());
//Print table to the web page
echo "<div class='CSSTableGenerator' >";
...
while($row = mysqli_fetch_array( $result )) {
	...
	echo "<td><center>";
	echo '<a href="cardtotal.php?tid='.$row['tid'].'">'.$row['tid'].'</font></a>';
	echo "</td>"; 
