...
$sql = "SELECT * FROM `tournament` WHERE tournament.uid = '$userid' ORDER by `id` desc LIMIT 0 , 1 ";
  ...
  //echo $data['uid'];
		...
		die('Error 1 - Select Tournament: '.mysqli_error());
			}
...
".(( !$_POST['total'] ) ? "" : "`total` = '".$_POST[total]."', ")."
".(( !$_POST['tid'] ) ? "" : "`tid` = '".$_POST[tid]."', ")."
`spam` = '1'


WHERE `pid` = '".$_POST['pid']."' AND `tmid` = '".$_POST['tmid']."' AND `jv` <> '".yes."' ";
...
$sql = "SELECT scores.tmid, scores.pid, roster.pid, 
  ...
  $result = mysqli_query($conn,$sql)
  or die ('Error in Query Try Again 5 :--' . mysqli_error()); 
  while($row = mysqli_fetch_array( $result )) 
    {
      echo $row['player_1']; 
    }
...
$sql = "SELECT scores.tmid, scores.pid, roster.pid, 
      ...
      WHERE scores.pid = '$pid' LIMIT 1";
  $result = mysqli_query($conn,$sql)
  or die ('Error in Query Try Again 6 :--' . mysqli_error()); 
  while($row = mysqli_fetch_array( $result )) 
    {
      echo $row['pid']; 
    }
