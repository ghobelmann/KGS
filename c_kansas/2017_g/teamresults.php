...
echo '<h2><center>'.$tname.'------------------------------------Page auto refreshes every 7 minutes.</h2></center>';
for($i=1; $i<=18; $i++)
...
$query = "SELECT *, roster.pid, roster.player_1, team.tmid, team.school FROM `scores` 
INNER JOIN roster ON scores.pid = roster.pid 
LEFT JOIN team on scores.tmid = team.tmid
WHERE `tid` = '".$tid."' AND dq != 'dq' AND dq != 'wd' AND total > '0' AND `jv` <> '".yes."'";
if(@!$result = mysqli_query($conn,$query))
...
$running_score[$player_i] = array("name" => $scores['player_1'], "team" => $scores['abv'],
