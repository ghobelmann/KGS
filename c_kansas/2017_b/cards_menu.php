...
while($row = mysqli_fetch_array( $result )) {
	...
	echo "<td><center>";
	echo '<a href="card.php?card='.$row['card'].'">'.$row['card'].'</font></a>';
	echo "</td>"; 
...
from scores
INNER JOIN tournament ON scores.tid = tournament.id
WHERE '$trny' = tournament.id AND scores.uid = '$userid'  ORDER by scores.tmid ASC, scores.pid ASC";
$result = mysqli_query($conn,$sql)
