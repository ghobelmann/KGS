...
$tid = $_GET['tid'];
  ...
  $output = "http://admin.kansasgolfscores.com/playerQRcodes.php?tid=$tid";
...
//Submit Query to MySql Database
  ...
  echo "<center><img src='http://www.kansasgolfscores.com/vendor/QR/php/qr_img.php?d=$output'>";
   echo "<br><h1>";
  echo $name;
   echo "<hr>";
...
while($row = mysqli_fetch_array( $result )) {
	...
	echo "<tr> <center> <h1>";
	echo '<a href="mobileEntryFront.php?tid='.$row['tid'].'&db='.$db.'&card='.$row['card'].'">Card '.$row['card'].'</font></a>';
	echo "</tr>";
} 
echo "</table>";
