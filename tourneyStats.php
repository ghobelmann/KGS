...
while($row = mysqli_fetch_array( $result )) {
  ...
  echo '<span class="clicker"><a href="playerStats.php?pid='.$row['pid'].'&tid='.$row['tid'].'">'.$row['player_1'].'</font></a>';

 	echo "</td></b><td><CENTER>"; 
  
  echo '<span class="clicker"><a href="teamStats.php?tmid='.$row['tmid'].'&tid='.$row['tid'].'">'.$row['school'].'</font></a>';
	echo "</td><td><CENTER>"; 
...
while($row = mysqli_fetch_array( $result )) {
  ...
  echo "</td><td><CENTER>"; 
	...
	echo "</td><td><CENTER>"; 
	echo '<a href="teamStats.php?tmid='.$row['tmid'].'">'.$row['school'].'</a>';
  echo "</td><td><b><CENTER>"; 
  echo $row['dq'];
  echo "</td>"; 
   $i++;
